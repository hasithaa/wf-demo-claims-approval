...
|---|---|---|
| `wso2-integration-control-plane-2.0.0-SNAPSHOT.zip` | `hasithaa/integration-control-plane`, local merge `icp-demo-obs` = main + `workflow-instance-graph` (PR wso2#851) + `icp-connection-hardening` (PR wso2#859) | `fed1d92b9` — 2026-09-08 (project Workflow page is a per-integration stats table with the project-wide pending-task total; project Human Tasks page is a per-integration summary of what waits for the caller, each row opening that integration's queue; the integration overview shows the same figures in place of the running-instances list; review shows its decision; a task's fail is a decision card; Add Runtime no longer special-cases workflow integrations; serialized heartbeat transaction, ballerina-library#9129) |
