...
|---|---|---|
| `wso2-integration-control-plane-2.0.0-SNAPSHOT.zip` | `hasithaa/integration-control-plane`, local merge `icp-demo-obs` = upstream main (which now carries wso2#859, merged) + `workflow-instance-graph` (PR wso2#851) | `d179d05ae` — 2026-09-10 (every timestamp is YYYY-MM-DD HH:mm:ss on one console-wide clock, local or UTC, switched in the header, with the UTC instant in the tooltip; Workflows and Human Tasks sit in a Manage group ahead of Observability; the project tables drop the Runtime column and an offline runtime's row says so in place of its figures; the integration overview's figures follow the selected workflow definition and its State/Worker Count cells are gone; Start Workflow separates the workflow's input from an Advanced section for Workflow ID and Timeout; project Workflow page is a per-integration stats table with the project-wide pending-task total; project Human Tasks page is the caller's own queue counted per integration — Task Count split into review tasks and human tasks, each row opening that queue; the integration overview shows the same figures in place of the running-instances list; review shows its decision; a task's fail is a decision card; Add Runtime no longer special-cases workflow integrations; serialized heartbeat transaction, ballerina-library#9129) |
